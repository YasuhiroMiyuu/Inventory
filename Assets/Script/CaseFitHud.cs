...
namespace CaseFit
{
    public class CaseFitHud : MonoBehaviour
    {
        ...
        void HandleRemaining(int remaining) => Set(remainingLabel, remaining + " ชิ้น");

        void HandleCleared(int stars)
        {
            string filled = new('★', stars);
            string empty = new('☆', 3 - stars);
            Set(messageLabel, "ผ่านด่าน  " + filled + empty);
        }

        void HandleFailed() => Set(messageLabel, "หมดเวลา");

        void HandleStuck() => Set(messageLabel, "ไม่เหลือที่ให้วางแล้ว ลองกดล้างกระเป๋า");
